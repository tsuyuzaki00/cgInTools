...
class SelfObject(bLB.SelfOrigin):
    def __init__(self):
...
class SelfBoneArmature(bLB.SelfOrigin):
    def __init__(self):
        super(SelfBoneArmature,self).__init__()
        self._setChoices+=[
            "Armature",
            "Bone",
            "Head",
    ...
    def rename_edit_str(self,bone_EditBone,rename):
        if not rename is None:
            bone_EditBone.name=rename
        return bone_EditBone.name
    ...
    ##PoseBone
    def selectPoseBone_query_PoseBone(self,armature_str,bone_str):
        armature_Object=bpy.data.objects[armature_str]
        bpy.context.view_layer.objects.active=armature_Object
    ...
    def boneConstraint_edit_func(self,bone_PoseBone,boneConstraint,attr_str,value):
        constraint_PoseBoneConstraints=bone_PoseBone.constraints[boneConstraint]
        if isinstance(value,str):
            exec("constraint_PoseBoneConstraints."+attr_str+"='"+str(value)+"'")
        else:
            exec("constraint_PoseBoneConstraints."+attr_str+"="+str(value))
    ...
    def setRename(self,variable):
        self._rename_str=variable
        return self._rename_str
    def getRename(self):
        return self._rename_str
    ...
    def currentValue(self):
        ...
        self._value=self.boneConstraint_query_value(bone_PoseBone,self._boneConstraint_str,self._attr_str)
        return self._value
    def getValue(self):
        return self._value
    ...
    def rename(self,rename=None,boneName=None,armatureName=None):
        _armature_str=armatureName or self._armature_str
        _bone_str=boneName or self._bone_str
        _rename=rename or self._rename_str

        bone_EditBone=self.selectEditBone_query_EditBone(_armature_str,_bone_str)
        rename_str=self.rename_edit_str(bone_EditBone,_rename)
        return rename_str
...
class SelfModifiersMesh(bLB.SelfOrigin):
    def __init__(self):
...
class SelfVertexWeight(bLB.SelfOrigin):
    def __init__(self):
    ...
    def convertMeshObject_create_vertexGroup(self,mesh_Object,subject_str):
        mesh_vertexGroup=mesh_Object.vertex_groups[subject_str]
    ...
    def editWeight(self,mesh=None,vertexGroupName=None,vertexID=None,weight=None):
        _object_str=mesh or self._object_str
        _subject_str=vertexGroupName or self._subject_str
...
class SelfMeshWeight(bLB.SelfOrigin):
    def __init__(self):
